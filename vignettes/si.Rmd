...
plot(od_calculated["res"], logz = TRUE)
```
...
scale_factor = sum(od_interzonal$Flow) /
  sum(od_calculated_interzonal$res)
od_calculated_interzonal = od_calculated_interzonal %>% 
  mutate(res_scaled = res * scale_factor)
...
od_joined %>% 
  ggplot() +
  geom_point(aes(Flow, res_scaled))
cor(od_joined$Flow, od_joined$res_scaled)^2
```
...
To do that, we'll first calculate the total number of people leaving each zone and then use the `constraint_p` argument:
...
od_constrained_p %>% 
  ggplot() +
  geom_point(aes(Flow, res))
cor(od_constrained_p$Flow, od_constrained_p$res)^2
```

Progress!
We have more than doubled the predictive ability of our model by using a 'production constrained' SIM, as defined mathematically in the [`sim-first-principles` vignette](https://robinlovelace.github.io/si/articles/sims-first-principles.html).
...
```{r}
f = Flow ~ (distance_euclidean * a)^b
m = nls(
  formula = f,
  data = od_originating,
  start = list(b = 0.8, a = 0.001),
  upper = c(5, 100), lower = c(-5, 0.00001),
  algorithm = "port"
  )
m
od_trained = si_predict(od_originating, model = m,
  constraint_p = originating_per_zone)
cor(od_trained$Flow, od_trained$res)
```
