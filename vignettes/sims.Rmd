...
For the purposes of this project, we will focus on production-constrained SIMs. These can be defined as follows [@wilson_family_1971]:
...
Before using the functions in this or other packages, it may be worth implementing SIMs from first principles, to gain an understanding of how they work. The code presented below was written before the functions in the `si` package were developed and demonstrate a common way of running SIMs, in a for loop, rather than using vectorised operations (used in the `si` package) which can be faster.
...
```{r inputs}
zones = si::zones
centroids = si::centroids
od_census_min = si::od_census_min
