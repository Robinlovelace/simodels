...
As documented in the [sim vignette](https://robinlovelace.github.io/sim/articles/sim.html), the function allows you to use any variable in the origin or destination data by joining all attributes onto the OD data frame, with column names appended with `origin` and `destination`.
...
For details on what SIMs are and how they have been defined mathematically and in code from first principles, see the [`sims` vignette](https://robinlovelace.github.io/sim/articles/simms-first-principles.html).

To dive straight into using {sim} to develop SIMs, see the [`sim` Get started vignette](https://robinlovelace.github.io/sim/articles/sim.html).
