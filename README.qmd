...
The goal of {si} is to provide a simple, '[tidy](https://dplyr.tidyverse.org/)', and flexible framework for developing spatial interaction models (SIMs), which estimate the amount of movement between spatial entities.
...
The approach works equally well when origin and destination points are different, as shown in the following example which estimates the number of trips to pubs in Leeds, assuming that for every trip to the pub there are 50 trips to work:
